...
def index_calc(x,y,z):
    ...
    xmax = 5
    ymax = 5.0/np.sqrt(3)
    ymaxind = int(np.floor(ymax/dy) + 1)
    totalslicep = 0
    ...
    for yj in range(int(yind)):
        index += np.floor((xmax - yj*dy*np.sqrt(3))/dx) + 1
    xind = np.round((x - yind*dy*np.sqrt(3))/dx)
    if(xind < 0): xind = 0
    index += xind
    return int(index)
...
def lookup(Pf,x,y,z):
    ...
    if(angle <= 30):
        ...
        yeval = final[1];
        zeval = final[2];
    index = index_calc(xeval,yeval,zeval)
    val = Pf[index]
    return val

def smoothening(x,x0,alpha):
